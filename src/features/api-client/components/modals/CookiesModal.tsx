...
export function CookiesModal() {
  ...
  const domains = Object.keys(jar).filter((d) => jar[d]?.length).sort();
      ...
      <div className="md-modal" style={{ width: 560, maxWidth: '92vw' }}>
        ...
        <div className="md-modal-body">
          <p className="cookies-intro">
            ...
            {domains.length > 0 && (
              <button className="md-text-btn" onClick={() => clearCookies()} style={{ height: 28 }}>Clear all</button>
            )}
          </p>
          {domains.length === 0 && <div className="cookies-empty">No cookies yet. Send a request that sets cookies.</div>}
          {domains.map((domain) => (
            <div key={domain} className="cookies-domain">
              <div className="cookies-domain-head">
                <span className="cookies-host">{domain}</span>
                <button className="md-icon-btn-small" title={`Clear cookies for ${domain}`} aria-label={`Clear cookies for ${domain}`} onClick={() => clearCookies(domain)}>
