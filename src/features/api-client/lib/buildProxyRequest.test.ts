...
const vars: EnvVar[] = [{ id: '1', key: 'base', value: 'https://api.x' }, { id: '2', key: 'tok', value: 'T9' }];

const tab = (over: Partial<TabState> = {}): TabState => ({
  id: 'r', method: 'GET', url: '{{base}}/users', params: [], headers: [],
...
describe('buildProxyRequest', () => {
  ...
  it('applies basic auth (base64) and apikey in query vs header', () => {
    const basic = buildProxyRequest(tab({ auth: { ...tab().auth, type: 'basic', basicUser: 'u', basicPass: 'p' } }), vars);
    expect(basic.headers.get('Authorization')).toBe(`Basic ${btoa('u:p')}`);
    const q = buildProxyRequest(tab({ auth: { ...tab().auth, type: 'apikey', apiKeyName: 'k', apiKeyValue: 'v', apiKeyIn: 'query' } }), vars);
    expect(q.finalUrl).toContain('k=v');
    const h = buildProxyRequest(tab({ auth: { ...tab().auth, type: 'apikey', apiKeyName: 'X-Key', apiKeyValue: 'v', apiKeyIn: 'header' } }), vars);
    expect(h.headers.get('X-Key')).toBe('v');
  });

  it('builds a raw JSON body with Content-Type for POST', () => {
    const b = buildProxyRequest(tab({ method: 'POST', body: { type: 'raw', rawContent: '{"a":{{tok}}}', rawType: 'application/json', formdata: [], urlencoded: [] } }), vars);
  ...
  it('flags form-data so the caller can drop Content-Type', () => {
    const b = buildProxyRequest(tab({ method: 'POST', body: { type: 'formdata', formdata: [{ id: 'k', key: 'f', value: 'x', type: 'text' }], urlencoded: [], rawContent: '', rawType: 'application/json' } }), vars);
